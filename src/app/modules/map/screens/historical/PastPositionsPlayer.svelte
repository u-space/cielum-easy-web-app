<script lang="ts">
	...
	$: {
		if ($queryPosition.isSuccess && $queryPosition.data?.data && $queryPosition.data?.data.length > 0 && currentTime) {
			...
			for (const position of $queryPosition.data.data) {
				...
				fetchedVehiclePositions.set(id, fetchedVehiclePositions.has(id) ? [...fetchedVehiclePositions.get(id)!, position] : [position]);
			}
	...
	$: lastPositionTime = $queryPosition.data?.data[$queryPosition.data?.data.length - 1]?.time_sent.getTime();
...
{#if currentTime > 0}
	<h1 id="time">{(new Date(currentTime)).toLocaleString(
		[],
		{
			year: 'numeric',
			month: 'numeric',
...
{#if $queryPosition.isSuccess && $queryPosition.data.data && $queryPosition.data.data.length > 0}
	...
	<div class="actions">
		...
		<input type="range" class="seek" min={$queryPosition.data.data[0].time_sent.getTime()}
		...
		<CButton icon="fast-forward-duotone" disabled={factor === 10} on:click={() => factor = 10}>
			x10
		</CButton>
		<CButton icon="fast-forward-duotone" disabled={factor === 100} on:click={() => factor = 100}>
			x100
		</CButton>
	</div>
{/if}
